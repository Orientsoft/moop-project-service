...
deploy: push
	...
	sed -i 's/{IMAGE_TAG_for_change}/$(TAG)/g' deploy/project-service-deployment.yaml
	# apply change
	kubectl apply -f deploy/ --namespace "$(DEPLOY_NAMESPACE)"
	# restore deployment.yaml
	sed -i 's/$(TAG)/{IMAGE_TAG_for_change}/g' deploy/project-service-deployment.yaml
